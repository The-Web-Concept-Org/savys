        ...
        // });//ajax call }
         $.ajax({
            ...
            success:function (response) {
                ...
                $("#get_product_name").empty().html("<option value="+response.id+">"+response.product_name+"</option>");
               $("#instockQty").html("instock :"+response.qty);
              console.log("<option value="+response.id+">"+response.product_name+"</option>");
            ...
            if (payment_type=="cash_in_hand" || payment_type=="credit_sale") {
                ...
                $("#get_product_name").empty().html("<option value="+response.id+">"+response.product_name+"</option>");
                //console.log("<option value="+response.product_id+">"+response.product_name+"</option>");
             }
...
//   var sl_price=  $('#get_product_name :selected').data('price');
var current_rate=$('#get_sale_price').val();
 
}
   

  if (id!='' && product_quantity!='' && max_qty>=product_quantity && price!='') {
...
// }
  ...
  if($('#product_idN_'+id).length){
    ...
    $(".product_ids").each(function(index){
      ...
      if (val==id) {
          if (pro_type=="add") {
          var Currentquantity=parseFloat(quantity)+parseFloat(product_quantity);
        }else{
          var Currentquantity=parseFloat(product_quantity);
        }
     ...
     Swal.fire({
      ...
      if (result.isConfirmed) {
          $.ajax({
                  url: 'php_action/custom_action.php',
                  type: 'post',
                  data: {setCheckStatus: id,status:'1'},
                  dataType: 'json',
                  success:function(res) {
                  sweeetalert(res.msg,res.sts,1500);
                  if(res.sts=="success"){
           ...
           $.ajax({
                  url: 'php_action/custom_action.php',
                  type: 'post',
                  data: {setCheckStatus: id,status:'3'},
                  dataType: 'json',
                  success:function(res) {
                  sweeetalert(res.msg,res.sts,1500);
                  if(res.sts=="success"){
